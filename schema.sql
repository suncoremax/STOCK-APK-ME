...
-- ══════════════════════════════════════════════════════════════════
--  V21: Attendance Punch + Live Location + Reward/Penalty Ledger
--  (Blueprint §14 + §15 — additive, safe to run on the live DB)
-- ══════════════════════════════════════════════════════════════════
...
-- One punch per user per day. role is manager/dsr/so (owner does not punch).
CREATE TABLE attendance (
  id         UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  user_key   TEXT        NOT NULL,
  user_name  TEXT        DEFAULT '',
  role       TEXT        NOT NULL CHECK (role IN ('manager','dsr','so')),
  ...
  lng        NUMERIC(10,6),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE (user_key, punch_date)
);
CREATE INDEX idx_att_user_key   ON attendance(user_key);
CREATE INDEX idx_att_punch_date ON attendance(punch_date);

-- Only meaningful for manager/dsr today (SO is tracked but has no reward math).
-- cleared_at is a cursor, not a delete: reward totals only ever sum attendance
-- rows AFTER this timestamp within the same period, so history is never lost.
...
CREATE INDEX idx_rl_user_period ON reward_ledger(user_key, period);

-- One row per user, upserted on every location ping — live snapshot only.
...
CREATE POLICY "srv_reward_ledger"   ON reward_ledger   FOR ALL TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY "anon_deny_reward_ledger"  ON reward_ledger  FOR ALL TO anon USING (false);
