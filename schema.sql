-- ══════════════════════════════════════════════════════════════════
--  AXIION স্টক ম্যানেজমেন্ট — Supabase Schema V21
--  Miron Electronics
--
...
-- Daily punch log — one row per user per day (UNIQUE constraint below).
-- status: 'present' (punched at/before 08:30) or 'late'.
-- at_office / distance_m are informational only (computed against
-- office_location if the punch included GPS) — they do NOT block the punch.
...
-- Marks when the Owner last "cleared/paid" a user's running reward total.
-- Reward math (in api/attendance.js) only counts attendance rows AFTER
-- cleared_at for the current month — nothing in `attendance` is deleted.
