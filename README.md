...
### Version beta v1 (V58 update)

A mobile-first stock & sales management web app built with **Vercel serverless functions** + **Supabase** (PostgreSQL).

---

## 🆕 What's New in V58 — SO stock-remaining on Today's Sale + Owner "No online payment today" with missing-day tracking

Still exactly the **12 API files** — no new endpoints. One small,
additive schema change (a widened CHECK constraint, no data touched).

1. **SO Dashboard → "📦 আজকের মোট বিক্রয়" — current remaining stock
   added beside each sold product.** Purely additive: the existing
   NET-sold-pieces figure and its calculation method are completely
   unchanged. Under each product's name, a new line now shows
   "বর্তমান স্টক অবশিষ্ট: N পিস", read straight from the same
   `stockMap` (`productId → currentStock`) the SO dashboard payload
   already carries — no new API field, no schema change.

2. **Owner Dashboard → "💵 অনলাইন জমার পরিমাণ" ("Today's Online
   Sell") — "No online payment today" option + retroactive
   missing-day flagging.**
   - A new **🚫 আজ কোনো জমা হয়নি** button sits beside the existing
     "➕ নতুন জমা যোগ করুন" button. Tapping it saves a zero-amount
     `no_payment`-tagged entry for today — a day answered this way
     shows "🚫 আজ কোনো অনলাইন জমা হয়নি" instead of a ৳ figure.
   - If the Owner has made **no** entry at all for a date — neither an
     amount nor "no payment" — today's box now shows
     "⚠️ Owner আজ এখনো কোনো পরিমাণ নির্বাচন করেননি" instead of
     silently showing ৳0 (which used to look identical to "zero was
     deposited").
   - This is **retroactive, per date**: opening the "এই মাসের মোট
     জমা" breakdown modal now also lists every past day in the
     pay-cycle (up to today) that's still pending or was marked
     "no payment" — e.g. if 27 Jul was filled in but 28 Jul was
     skipped, 28 Jul shows up flagged even after the month moves on.
   - If an amount **is** entered for a day, it displays exactly as
     before — this change only adds the two missing states, it
     doesn't touch the existing amount path.
   - ⚠️ **SQL migration required** — see
     `AXIION_V58_partial_migration.sql` (provided separately, outside
     the deployable zip) for the one-line CHECK-constraint widening
     needed on your existing live database
     (`online_deposit.deposit_method` gains `'no_payment'` as a third
     allowed value). `schema.sql` in this package already includes it
     for fresh installs.

3. **Version indicator.** The app now visibly shows its version
   (`V58`) on the splash screen, the PIN screen, and at the bottom of
   the side menu — previously no version number was shown anywhere in
   the UI itself.
