const { supabase, cors, num, now_, mapPayment, mapOrder, safeErr } = require('./_lib/db');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && action === 'dsr-reconcile') {
      ...
      const { data: allDueTx, error: allDueErr } = await supabase.from('transactions')
        .select('type,total_revenue').eq('sr_id', dsrId).in('type', ['give', 'return']);
      if (allDueErr) throw allDueErr;
      ...
      const { data: allPay, error: allPayErr } = await supabase.from('sr_payments').select('amount').eq('sr_id', dsrId);
      ...
      const { data: dayTx, error: dayTxErr } = await supabase.from('transactions')
        .select('type,total_revenue,total_cost').eq('sr_id', dsrId).eq('date', d)
      ...
      const damageToday      = sum('damage', 'total_cost');
