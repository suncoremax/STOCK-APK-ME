  ...
  try {
    ...
    if (req.method === 'POST' && action === 'order_submit') {
      ...
      const { data, error } = await supabase.from('orders').insert({
        ...
        status: 'pending', load_status: 'not_started', load_ticks: {},
        created_at: now_()
      }).select().single();
      if (error) throw error;
    ...
    if (req.method === 'POST' && action === 'order_approve') {
      ...
      if (!id || !dsrId) return res.json({ ok: false, error: 'id ও dsrId প্রয়োজন' });
      ...
      const { error } = await supabase.from('orders').update({
        ...
        status: 'approved', assigned_dsr_id: String(dsrId), load_status: 'not_started', load_ticks: {},
