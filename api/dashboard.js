...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (role === 'so' && userId) {
      ...
      // Update #25 — "you're due at [road] today" (day 1: SO visits).
      const { data: roadDutyRows } = await supabase.from('road_visit_plans')
        .select('*').eq('so_id', userId).eq('so_visit_date', bdtToday());
      const roadDuty = (roadDutyRows || []).map(mapRoadPlan);

      // ── V55 #56 — "আজকের মোট বিক্রয়" (Today's Total Sale) widget ──
      // Replaces the removed "আমার DSR" name-list tab. RAW pieces sold
      // today by this SO's assigned DSRs only (give − return units from
      // `transactions`), per product — deliberately excludes the `bonus`
      // table's buy-X-get-Y free units, which stay a separate concept.
      // SO's own point-sale is NOT counted here (this reflects what the
      // DSRs actually sold/delivered, not the SO's own direct sale).
      const productThumbMap = {}, productNameMap = {};
      products.forEach(p => { productThumbMap[p.id] = p.thumb || ''; productNameMap[p.id] = p.name || ''; });
      const todayProdMap = {};
      txToday.forEach(r => {
        if (dsrIds.indexOf(r.srId) === -1) return;
        if (r.type !== 'give' && r.type !== 'return') return;
        ...
        todayProdMap[pid].sold += (r.type === 'give' ? units : -units);
      });
    ...
    if (role === 'dsr' && userId) {
      ...
      // Update #25 — "you're due at [road] today" (day 2: paired DSR delivers,
      // always so_visit_date + 1 for the same road).
      const { data: roadDutyRows } = await supabase.from('road_visit_plans')
        .select('*').eq('dsr_id', userId).eq('dsr_visit_date', bdtToday());
      const roadDuty = (roadDutyRows || []).map(mapRoadPlan);
