...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'load-all') {
      ...
      // V39: stock comes straight from products.current_stock (kept in
      ...
      // migration_v39_stock_balance.sql.
      const stockMap = {};
      products.forEach(p => { stockMap[p.id] = p.currentStock; });
      return res.json({ ok: true, products, srs: srsAll, stockMap });
    }
    ...
    if (role === 'so' && userId) {
      ...
      // V39: stock read straight from products.current_stock — see
      // migration_v39_stock_balance.sql — no more full transaction
    ...
    // V39: the old 'txAll' full-lifetime-transactions fetch below used
    ...
    // in sync by a DB trigger — see migration_v39_stock_balance.sql),
