...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════
    if (action === 'load-all') {
      const [pRes, sRes, tRows] = await Promise.all([
        supabase.from('products').select('*').order('created_at'),
        supabase.from('srs').select('*').order('created_at'),
        fetchAll(() => supabase.from('transactions').select('tx_id,type,product_id,total_units').order('created_at'))
      ]);
      const products = (pRes.data || []).map(mapProduct);
      const srsAll   = (sRes.data || []).map(mapSR);
      const allTx    = (tRows || []).map(r => ({
    ...
    if (role === 'so' && userId) {
      ...
      const [prodRes, txStockRows] = await Promise.all([
        supabase.from('products').select('*').order('created_at'),
        fetchAll(() => supabase.from('transactions').select('tx_id,type,product_id,total_units').order('created_at'))
      ]);
      const products = (prodRes.data || []).map(mapProduct);
      const stockMap = calcStock((txStockRows || []).map(r => ({
        txId: String(r.tx_id || ''), type: r.type || '',
        productId: String(r.product_id || ''), totalUnits: String(r.total_units || 0)
      })));
    ...
    if (role === 'dsr' && userId) {
      ...
      const [prodRes, txStockRows, txAll, paymentsRes, txMonthRes, payMonthRes] = await Promise.all([
        supabase.from('products').select('*').order('created_at'),
        fetchAll(() => supabase.from('transactions').select('tx_id,type,product_id,total_units').order('created_at')),
    ...
    const [pRes, sRes, txTodayRes, txMonthRes, txAll, payMonthRes, dmgRes, bonRes] = await Promise.all([
      supabase.from('products').select('*').order('created_at'),
      supabase.from('srs').select('*').order('created_at'),
      ...
      // Lifetime, all-products stock + bonus-window aggregate — the single
      // most important query to paginate. Note: 'date' is included here
      // (it was missing before), which had silently broken the bonus-
      // pending calc below (r.date was always undefined, so the bonus
      // "since last cleared" filter never matched anything).
    ...
    txAll.forEach(r => {
      ...
      // V35 — damage no longer touches stock (report/reimbursement only).
