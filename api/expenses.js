...
// V59 — re-verifies an Owner PIN server-side (never trust a client-side
// role flag alone). Same table/pattern already used by api/shops.js,
// api/attendance.js, api/sr-payments.js (action=approval_edit) and
// api/report.js (action=daily-generate). Restores PIN-gating on the
// Online Deposit box, which V58 had left checking only `requesterRole`.
  ...
  try {
    ...
    if (req.method === 'GET' && action === 'deposit-get') {
      ...
      const { data: todayRows, error: e1 } = await supabase.from('online_deposit')
        .select('id,amount,deposit_method,set_at').eq('date', d);
      if (e1) throw e1;
      ...
      // V59 — only ever expose per-entry ids/edit-ability for the actual
      // current BDT day. If someone requests a past `date`, `entries` is
      // omitted — that day's state is locked and has nothing to edit.
      const entries = (d === bdtToday())
        ? rows.map(r => ({ id: r.id, amount: num(r.amount), method: r.deposit_method || 'bank', setAt: r.set_at || '' }))
        : undefined;
      return res.json({ ok: true, today: todayAmt, month: monthAmt, date: d, todayStatus, isToday: d === bdtToday(), entries });
    }
    ...
    if (req.method === 'POST' && action === 'deposit-set') {
      ...
      // V59 — restored: touching the Online Deposit box (adding an
      // amount, or marking "no online payment today") requires the
      // Owner's PIN to be re-entered, same as every other sensitive
      // Owner action (shop edit/delete, office location, approval
      // edits...). A `role==='owner'` client flag alone is never
      // trusted for a money-affecting write.
    ...
    // V59 — undo a single entry saved earlier TODAY. Only today's date is
    // ever editable this way; the moment BDT midnight passes, that day's
    // last-recorded state (whatever entries/markers exist for it) is
    // permanently locked and this will refuse the delete. This is what
    // lets the Owner correct a mistake ("add ৳5,000... no wait, delete
    // that") any number of times during the same day, while a past day's
    // record can never quietly change after the fact.
    if (req.method === 'DELETE' && action === 'deposit-delete') {
      const d = req.body || {};
      const requesterRole = String(d.requesterRole || req.query?.requesterRole || '').trim();
      if (requesterRole !== 'owner') return res.json({ ok: false, error: 'শুধু মালিক এন্ট্রি মুছতে পারবেন' });
      if (!(await _verifyOwnerPin(d.ownerPin || req.query?.ownerPin))) return res.json({ ok: false, error: 'ভুল Owner PIN' });
      const id = d.id || req.query?.id;
      if (!id) return res.json({ ok: false, error: 'এন্ট্রি আইডি প্রয়োজন' });
      const { data: row, error: e1 } = await supabase.from('online_deposit').select('date').eq('id', id).limit(1);
      if (e1) throw e1;
      if (!row || !row.length) return res.json({ ok: false, error: 'এন্ট্রি পাওয়া যায়নি' });
      if (row[0].date !== bdtToday()) {
        return res.json({ ok: false, error: 'আজকের আগের দিনের এন্ট্রি আর পরিবর্তন করা যাবে না — সেই দিনের রেকর্ড চূড়ান্ত' });
      }
      const { error } = await supabase.from('online_deposit').delete().eq('id', id);
      if (error) throw error;
      return res.json({ ok: true });
    }
    // V59 — wipe TODAY's entries entirely (back to "pending", as if the
    // Owner hadn't touched the box yet). Same today-only lock as above.
    if (req.method === 'POST' && action === 'deposit-reset-today') {
      const d = req.body || {};
      const requesterRole = String(d.requesterRole || '').trim();
      if (requesterRole !== 'owner') return res.json({ ok: false, error: 'শুধু মালিক রিসেট করতে পারবেন' });
      if (!(await _verifyOwnerPin(d.ownerPin))) return res.json({ ok: false, error: 'ভুল Owner PIN' });
      const t = bdtToday();
      const { error } = await supabase.from('online_deposit').delete().eq('date', t);
      if (error) throw error;
      return res.json({ ok: true });
    }
