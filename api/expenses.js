const {
  ...
  cyclePeriodForDate, cyclePeriodBounds
} = require('./_lib/db');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── Update #54 — Daily Deposit tap-to-expand breakdown ──
    // GET /api/expenses?action=deposit-history&date=YYYY-MM-DD
    // Returns every online_deposit row within the current pay cycle
    // (same 26th→25th window as the running "this month" total above),
    // newest first, so tapping the month total shows a date-by-date
    // breakdown instead of just the single running figure.
    if (req.method === 'GET' && action === 'deposit-history') {
      const d = req.query.date || today();
      const monthStart = cyclePeriodBounds(cyclePeriodForDate(d)).start;
      const { data, error } = await supabase.from('online_deposit')
        .select('date,amount,set_by,set_at')
        .gte('date', monthStart).lte('date', d)
        .order('date', { ascending: false });
      if (error) throw error;
      const rows = (data || []).map(r => ({
        date: r.date, amount: num(r.amount), setBy: r.set_by || '', setAt: r.set_at || ''
      }));
      const total = rows.reduce((s, r) => s + r.amount, 0);
      return res.json({ ok: true, rows, total, periodStart: monthStart, periodEnd: d });
    }
