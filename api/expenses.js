...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── Daily Online Deposit (V31) — one shared amount/day for everyone ──
    if (req.method === 'GET' && action === 'deposit-get') {
      const d = req.query.date || today();
      const monthStart = d.slice(0, 7) + '-01';
      const { data: todayRow, error: e1 } = await supabase.from('online_deposit')
        .select('*').eq('date', d).limit(1);
      if (e1) throw e1;
      const { data: monthRows, error: e2 } = await supabase.from('online_deposit')
        .select('amount').gte('date', monthStart).lte('date', d);
      if (e2) throw e2;
      const todayAmt = (todayRow && todayRow[0]) ? num(todayRow[0].amount) : 0;
      const monthAmt = (monthRows || []).reduce((s, r) => s + num(r.amount), 0);
      return res.json({ ok: true, today: todayAmt, month: monthAmt, date: d });
    }
    if (req.method === 'POST' && action === 'deposit-set') {
      ...
      const { error } = await supabase.from('online_deposit').upsert({
        date: d.date, amount: num(d.amount),
    ...
    // Folded in here (instead of separate api/push-tokens.js) to stay
    // within Vercel's 12-Serverless-Function cap — same behavior as
    // NOTIFICATION_PROTOCOL.md §2, just reached via a different path:
    //   POST   /api/expenses?action=push-token-register   { userKey, role, token }
    //   DELETE /api/expenses?action=push-token-unregister  { token }
    if (req.method === 'POST' && action === 'push-token-register') {
      const d = req.body || {};
    ...
    if (req.method === 'DELETE' && action === 'push-token-unregister') {
      const d = req.body || {};
      const token = String(d.token || '').trim();
      if (!token) return res.json({ ok: false, error: 'token প্রয়োজন' });
    ...
    // ── Manual/external push send (V40) ────────────────────────────────
    // Folded in here instead of separate api/send-push.js, same reason.
    //   POST /api/expenses?action=push-send
    //   Headers: x-push-secret: <PUSH_SECRET>
    //   Body: { title, body, url?, image?, token? | userKey? | role? | topic? }
    if (req.method === 'POST' && action === 'push-send') {
      const secret = req.headers['x-push-secret'];
      if (!process.env.PUSH_SECRET || secret !== process.env.PUSH_SECRET) {
        return res.status(401).json({ ok: false, error: 'Unauthorized' });
      }
      const d = req.body || {};
      if (!d.title || !d.body) return res.status(400).json({ ok: false, error: 'title ও body প্রয়োজন' });
