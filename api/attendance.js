...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && action === 'punch') {
      ...
      let status = null;
      if (punchType === 'in') {
        // On-time window widened from a single 8:30 cutoff to a full
        // morning range: punching any time from 7:00 AM through 10:00 AM
        // counts as on-time; outside that window (too early or after
        // 10:00) is marked late. Evening checkout window is unchanged.
        const onTimeStartMin = 7 * 60;
        const onTimeEndMin = 10 * 60;
    ...
    if (req.method === 'POST' && action === 'advance-request') {
      ...
      const month = /^\d{4}-\d{2}$/.test(d.month || '') ? d.month : _tzISODate(new Date()).slice(0, 7);
    ...
    if (req.method === 'GET' && action === 'target-get') {
      const userKey = req.query.userKey;
      const period = req.query.period || _tzISODate(new Date()).slice(0, 7);
    ...
    // ══════════════════════════════════════════════
    //  PRODUCT-WISE SALES TARGET — owner also sets a quantity target
    //  (not money) per product each month, company-wide. Separate from
    //  the ৳ target above; achieved is summed live from transactions
    //  (total_units), same give/point_sale − return/point_damage_return
    ...
    // target qty + live achieved qty (read-only for everyone, editable
    // by owner only on the front-end).
    if (req.method === 'GET' && action === 'product-target-list') {
      const period = req.query.period || _tzISODate(new Date()).slice(0, 7);
      const { data: prods, error: pErr } = await supabase
        .from('products').select('id,name,unit_type').order('name');
      if (pErr) throw pErr;
      ...
      const achievedMap = await _achievedUnitsByProduct(period);
...
async function _achievedForUser(userKey, period) {
  ...
  const { data, error } = await supabase
    .from('transactions').select('type,total_revenue').eq('sr_id', userKey).gte('date', from).lte('date', to);
  if (error) throw error;
  let rev = 0;
  (data || []).forEach(r => {
...
// Sums total_units per product_id for the period, same give/point_sale −
// return/point_damage_return sign pattern as every other achieved-figure
// helper — backs the product-wise quantity target feature.
async function _achievedUnitsByProduct(period) {
  const [y, m] = period.split('-').map(Number);
  const lastDay = _daysInMonth(y, m - 1);
  const from = period + '-01', to = period + '-' + String(lastDay).padStart(2, '0');
  const { data, error } = await supabase
  ...
  (data || []).forEach(r => {
    ...
    if (!pid) return;
    if (r.type === 'give' || r.type === 'point_sale') map[pid] = (map[pid] || 0) + v;
    if (r.type === 'return' || r.type === 'point_damage_return') map[pid] = (map[pid] || 0) - v;
  });
