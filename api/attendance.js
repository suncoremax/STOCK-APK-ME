...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ══════════════════════════════════════════════
    //  SALES TARGETS (AXIION §16)
    //  Owner sets a period (YYYY-MM) revenue target per DSR/SO.
    ...
    // POST ?action=target-set — OWNER ONLY sets a target (per DSR/SO, or the
    ...
    if (req.method === 'POST' && action === 'target-set') {
      ...
      const role = userKey === 'COMPANY_TOTAL' ? 'company' : ((d.role === 'so' || d.role === 'dsr') ? d.role : '');
      const row = {
    ...
    //  owner/manager → every DSR/SO ; so → self + assigned DSRs ; dsr → self only
    if (req.method === 'GET' && action === 'target-list') {
      const period = req.query.period || _tzISODate(new Date()).slice(0, 7);
      const viewerRole = req.query.viewerRole || '';
      const viewerId = req.query.viewerId || '';

      const { data: allSrs, error: srsErr } = await supabase
        .from('srs').select('id,name,role,so_id,display_no').in('role', ['dsr', 'so']).order('display_no');
      if (srsErr) throw srsErr;

      let list = allSrs || [];
      if (viewerRole === 'so') list = list.filter(s => s.id === viewerId || String(s.so_id || '') === String(viewerId));
      else if (viewerRole === 'dsr') list = list.filter(s => s.id === viewerId);
      // owner / manager: no filter — full company list
